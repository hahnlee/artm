...
generated ownership table covers 112 of the pinned NDK r28c API-35 arm64
`libc++_shared.so`'s 160 libc-family `@LIBC` imports and all 18
unversioned `liblog.so` exports. The other 48 libc imports remain explicit in
...
numeric parsing, liblog, and DSO lifecycle. `generate_manifests.py` derives the
...
universe. 111 imports are owned by `libc.so`; loader-owned
...
checks 130 unique `(SONAME, symbol)` owners and all 48 unsupported libc
imports, rejects wrong SONAMEs and versions, performs 12-thread lookup stress,
routes all 130 entries through the typed adapters with exact per-provider
