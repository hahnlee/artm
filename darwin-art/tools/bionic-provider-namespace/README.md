...
generated ownership table covers 148 of the pinned NDK r28c API-35 arm64
`libc++_shared.so`'s 160 libc-family `@LIBC` imports and all 18
unversioned `liblog.so` exports. The other 12 libc imports remain explicit in
...
universe. 147 imports are owned by `libc.so`; loader-owned
...
supplies typed adapters and intentionally leaves exactly twenty-two resolver
...
checks 166 unique `(SONAME, symbol)` routes and all 12 unsupported libc
imports, rejects wrong SONAMEs and versions, performs 12-thread lookup stress,
routes all 166 entries through the typed adapters with exact per-provider
