...
generated ownership table covers 151 of the pinned NDK r28c API-35 arm64
`libc++_shared.so`'s 160 libc-family `@LIBC` imports and all 18
unversioned `liblog.so` exports. The other 9 libc imports remain explicit in
...
universe. 150 imports are owned by `libc.so`; loader-owned
...
supplies typed adapters and intentionally leaves exactly twenty-three resolver
...
checks 169 unique `(SONAME, symbol)` routes and all 9 unsupported libc
imports, rejects wrong SONAMEs and versions, performs 12-thread lookup stress,
routes all 169 entries through the typed adapters with exact per-provider
