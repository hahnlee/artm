...
generated ownership table covers 147 of the pinned NDK r28c API-35 arm64
`libc++_shared.so`'s 160 libc-family `@LIBC` imports and all 18
unversioned `liblog.so` exports. The other 13 libc imports remain explicit in
...
provider-local Android `FILE`, liblog, and DSO lifecycle.
...
universe. 146 imports are owned by `libc.so`; loader-owned
...
supplies typed adapters and intentionally leaves exactly twenty-one resolver
...
checks 165 unique `(SONAME, symbol)` routes and all 13 unsupported libc
imports, rejects wrong SONAMEs and versions, performs 12-thread lookup stress,
routes all 165 entries through the typed adapters with exact per-provider
