...
executable subset is `JavaVM::GetEnv` plus `JNIEnv::{GetVersion, FindClass, GetSuperclass,
...
thread attachment/detachment, local frames, object arrays, primitive-array
elements/pinning, class-loader policy, or method invocation support. A registered native
...
Only the successful `FindClass`, `RegisterNatives`, reference lifecycle,
modified-UTF-8 string, byte-array region, and bounded exception paths are
coherent in this slice. Their
