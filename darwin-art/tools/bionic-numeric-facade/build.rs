...
fn compile(source: &str, object: &Path, sdk: &str, includes: &[&str]) {
    ...
    if let Ok(sanitizer) = env::var("BIONIC_NUMERIC_C_SANITIZER") {
        command
...
fn main() {
    ...
    let archive = out.join("libbionic_numeric.a");
    assert!(
        Command::new("ar")
            .arg("rcs")
            .arg(&archive)
    ...
    println!("cargo:rustc-link-lib=static=bionic_numeric");
    if let Ok(sanitizer) = env::var("BIONIC_NUMERIC_C_SANITIZER") {
    ...
    println!("cargo:rerun-if-env-changed=BIONIC_NUMERIC_C_SANITIZER");
    for source in [
        "src/provider.c",
