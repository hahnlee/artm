...
fn main() {
    ...
    let shims = output_dir.join("time_shims.o");
    let errno = output_dir.join("errno_tls.o");
    let archive = output_dir.join("libdarwin_art_bionic_time.a");
    ...
    println!("cargo:rustc-link-lib=static=darwin_art_bionic_time");
    for source in [
        "src/shims.c",
