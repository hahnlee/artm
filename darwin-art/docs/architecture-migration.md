...
- Unmodified AOSP `004-JniTest` passes in interpreter, JIT, and comparison
...
- Checkpoint 1070 (2026-09-11): Chromium tab-graphics 경로를 새 app-data로
  검증해 real `TabSwitcherButtonView`/`TabGridView`, child SurfaceControl과
  GPU 상태를 확인했다(`RC=0`, `target-states=10`). 합성 MotionEvent의
  DOWN→UP 간격은 monotonic deadline으로 보장되며 실제 20.292ms/78.280ms를
...
- Checkpoint 967 (2026-09-10): foundation/runtime 분리 시도에서 patch-chain hunk
  경계가 build-foundation 변형과 맞지 않아 적용을 중단했다. 현재 baseline은 clean이며,
  다음 단계는 foundation shadow 기준으로 exact allocator diff를 재생성하는 것이다.
...
- Checkpoint 1076 (2026-09-11): Chromium `ACTION_VIEW https://example.com/`을
  변경 없이 재실행해 SurfaceFlinger/Metal scanout과 일반 창 캡처를 확인했다.
  `/tmp/chromium-example-normal.P4RM19/window.png`에서 Example Domain 본문과
...
- Checkpoint 1159 (2026-09-11): fresh ledger가 976/1,076개까지 진행되어
  970 passed/6 failed이며 identity `eb468bf1…` 단일성을 유지한다. 프로세스는
  정상 실행 중이고 남은 항목 완료 후 순차 재검증을 진행한다.
...
- Checkpoint 1204 (2026-09-11): JIT on/no-inline Chromium 실행이 실제
