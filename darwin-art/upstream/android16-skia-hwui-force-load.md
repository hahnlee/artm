...
`skia_include_multiframe_procs`, so upstream
`src/ports/SkFontMgr_custom_empty.cpp` and `tools/SkSharingProc.cpp` are real
members of `libskia.a`. The sharing implementation's PNG deserialize path is
built against the pinned AOSP libpng/zlib archives. No symbol is synthesized
after the build. Apple's ImageIO/CoreGraphics remain system image providers;
CoreText is neither linked nor imported.
...
in the link map, executes the empty font-manager/sharing-context smoke test,
